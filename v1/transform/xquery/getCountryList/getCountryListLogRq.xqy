...
declare function local:getCountryListLogRq($requestDatetime as xs:string, 
    ...
    <log:AuditoryLogRequest>
        ...
        <log:ServiceIdentification>
            ...
            <log:code>{xref:lookupXRef('common/CrossReference/transform/xref/TargetCrossInfo', 'cr_key', 'SUPPORT_GETCOUNTRYLIST_IDFUNCIONALITY', 'cr_value', false())}</log:code>
            {
...
local:getCountryListLogRq($requestDatetime, $requestHeader, $requestPayload, $targetRouting, $correlationID,$requestPayloadTarget, $countBusiness)