...
declare function local:getCustomerIdTypeListLogRq($requestDatetime as xs:string, 
    ...
    <log:AuditoryLogRequest>
        ...
        <log:ServiceIdentification>
            ...
            <log:code>{xref:lookupXRef('common/CrossReference/transform/xref/TargetCrossInfo', 'cr_key', 'SUPPORT_GETCUSTOMERIDTYPELIST_IDFUNCIONALITY', 'cr_value', false())}</log:code>
            {
...
local:getCustomerIdTypeListLogRq($requestDatetime, $requestHeader, $requestPayload, $targetRouting, $correlationID,$requestPayloadTarget, $countBusiness)