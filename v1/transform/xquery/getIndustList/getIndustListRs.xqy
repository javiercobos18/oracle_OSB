...
declare namespace sp="http://xmlns.oracle.com/pcbpel/adapter/db/sp/DB_PHOENIX_SP_USP_SRVOSB_ACTIVIDADECONOMICA";
(:: import schema at "../../../interfaces/schemas/DB_PHOENIX_SP_USP_SRVOSB_ACTIVIDADECONOMICA.xsd" ::)
...
declare function local:getIndustListRs($requestPayloadProxy as element() (:: schema-element(cat:getIndutListRq) ::), 
                                       $responsePayloadTarget as element() (:: schema-element(sp:OutputParameters) ::)) 
                                       as element() (:: schema-element(cat:getIndutListRs) ::) {
    ...
    <cat:getIndutListRs>
      ...
      </cat:HeaderResponse>
      {
        ...
        <cat:IndutList>
        {
        for $LR_INDUTLIST in $responsePayloadTarget/sp:RowSet0/sp:RowSet0_Row
        return
          <cat:IndutInfo>
              <cat:industNum>{fn:data($LR_INDUTLIST/sp:CodigoActEconomica)}</cat:industNum>
              <cat:desc>{fn:data($LR_INDUTLIST/sp:Descripcion)}</cat:desc>
          </cat:IndutInfo>
          }
      </cat:IndutList>
      else
...
local:getIndustListRs($requestPayloadProxy, $responsePayloadTarget)
