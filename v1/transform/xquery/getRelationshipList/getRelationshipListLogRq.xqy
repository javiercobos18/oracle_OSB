...
declare function local:getRelationshipListLogRq($requestDatetime as xs:string, 
    ...
    <log:AuditoryLogRequest>
        ...
        <log:ServiceIdentification>
            ...
            <log:code>{xref:lookupXRef('common/CrossReference/transform/xref/TargetCrossInfo', 'cr_key', 'SUPPORT_GETRELATIONSHIPLIST_IDFUNCIONALITY', 'cr_value', false())}</log:code>
            {
...
local:getRelationshipListLogRq($requestDatetime, $requestHeader, $requestPayload, $targetRouting, $correlationID,$requestPayloadTarget, $countBusiness)